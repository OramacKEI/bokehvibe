...
- **为何重要：** 管线最上游，输出相对视差 D。本项目用 **Small** 版（25M，仅推理）。
- **许可注意：** 仅 Small 模型是 Apache-2.0；Base/Large/Giant 为 CC-BY-NC-4.0（非商用）。用 Small 正好避开限制。
- arXiv：https://arxiv.org/abs/2406.09414
- 项目页：https://depth-anything-v2.github.io/
- 官方代码：https://github.com/DepthAnything/Depth-Anything-V2
...
### *Realistic Rendering of Bokeh Effect Based on Optical Aberrations* (The Visual Computer, 2010)
- **为何重要：** 像差→散景的早期建模工作。**必引并划清界限**：它是前向、非可微、非学习的；
  本项目的增量 = 可微（支撑训练+指纹反演）+ 轻量单图管线 + 解耦可控性监督。
- Springer：https://link.springer.com/article/10.1007/s00371-010-0459-5

### *Rendering Realistic Spectral Bokeh due to Lens Stops and Aberrations* (The Visual Computer, 2012)
- **为何重要：** 上篇的续作，光线追踪式像差散景渲染（含光谱/色差）。同样**必引并划清界限**
  （理由同上）；审稿人很可能拿这两篇质疑新颖性，related work 要先手处理。
- Springer：https://link.springer.com/article/10.1007/s00371-012-0673-4

### MPIB — *An MPI-based Bokeh Rendering Framework for Realistic Partial Occlusion Effects* (ECCV 2022)
- **为何重要：** 同作者组的多平面图像散景框架，分层渲染思路参考。
- 检索关键词："MPIB Peng ECCV 2022"

---

## 五、邻近领域：盲像差估计 / PSF 标定（RQ3 镜头指纹的 related work 必须覆盖）

> 这些工作从图像**估计/标定像差或 PSF**，目标是"矫正像差"；本项目的指纹反演方法相似、
> 方向相反（"复刻风格"）。论文 related work 不覆盖会被视为遗漏。
...
- **用途：** **RQ3 定量化的钥匙**——对两镜头各做指纹标定 → 渲染 lens-to-lens 转换 →
